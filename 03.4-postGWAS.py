...
if __name__ == "__main__":
    ...
    # 获取脚本的路径下supp文件下下面的Arial.ttf

    custom_font_path = Path(__file__).parent / "supp/Arial.ttf"
    ...
    # # only for rerun
    # if (saveDir / "h2_df.csv").exists():
    #     log.write(f"{trait} has been processed", verbose=verbose)
    #     exit(0)
    # #

    # getTraitDir = lambda sex: rootDir / f"{trait}.feather"
    TraitDir = rootDir / f"{trait}.feather"
    ...
    try:
        ...
        #             "N": n_case_female + n_control_female,
        #         },
        #         "Male": {
        #             "Case": n_case_male,
        #             "Control": n_control_male,
        #             "N": n_case_male + n_control_male,
        #         },
        #     }
        # else:
        #     trait_type = "qt"
        #     count_dict = {
        #         "Female": {"N": mysumstats.data["N"].iloc[0]},
    ...
    try:
        ...
        mysumstats.data["P"] = 10 ** (-mysumstats.data["MLOG10P"])
        # Male_mysumstats.data["P"] = 10 ** (-Male_mysumstats.data["MLOG10P"])

        mysumstats.data["Z"] = (
            mysumstats.data["BETA"] / mysumstats.data["SE"]
        )
        # Male_mysumstats.data["Z"] = (
            # Male_mysumstats.data["BETA"] / Male_mysumstats.data["SE"]
        # )
        mysumstats.data["MAF"] = mysumstats.data["EAF"].apply(
    ...
    try:
        Female_lambda = calculate_lambda_from_p(mysumstats.data["P"])
    ...
    try:
        ...
        if trait_type == "bt":
            title += f" (Case:{count_dict['Female']['Case']}, Control:{count_dict['Female']['Control']})"

            # title_male += f" (Case:{count_dict['Male']['Case']}, Control:{count_dict['Male']['Control']})"

        else:
            # title_male = f" Total N={count_dict['Male']['N']}"
            title = f" Total N={count_dict['Female']['N']}"

        mahanttanPlot = mysumstats.plot_mqq(
            skip=2, anno="GENENAME", title=title
        )
    ...
    try:
        sig_level = 5e-8
        mysumstats_lead_data = mysumstats.get_lead(
        ...
        Female_sig_loci_numbers = mysumstats_lead_data.data.shape[0]
        ...
        if Female_sig_loci_numbers == 0:
            ...
            mysumstats_lead_data.data.reset_index(drop=True).to_feather(
        ...
        mysumstats_lead_data_trumpetPlot = mysumstats_lead_data.plot_trumpet(
            # sig_level=5e-6,
            # p_level=5e-6,
            anno="GENENAME",
            anno_style="right",
        ...
        # )
        save_fig(
            mysumstats_lead_data_trumpetPlot,
            saveDir / "trumpetPlot",
            dpi=default_dpi,
        )
    except Exception as e:
    ...
    #         anno2="GENENAME",
    #         build="38",
    #         titles=["Male" + title_male, "Female" + title],
    #     )
    ...
    # try:
    #     ## extract inner set
    #     innerSet = mysumstats.data.merge(
    #         # Male_mysumstats.data,
    #         on=["SNPID", "CHR", "POS", "EA", "NEA", "rsID"],
    ...
    #     SigLoci = SexDiff_sumstats.get_lead(sig_level=0.05, anno=True, gls=True)

    #     if SigLoci.data.shape[0] == 0:
    #         log.write(f"-No significant loci found of {trait}", verbose=verbose)
    #         SigLoci.data.reset_index(drop=True).to_feather(
    #             saveDir / "sex_het_SigLoci.feather"
    #         )
    #     else:
    #         SigLoci.data.reset_index(drop=True).to_feather(
    #             saveDir / "sex_het_SigLoci.feather"
    ...
    try:
        ...
        mysumstats.data.reset_index(drop=True).to_feather(
            saveDir / "hm3.feather"
        )

        mysumstats.estimate_h2_by_ldsc(
            ref_ld_chr=ref_ld_chr, w_ld_chr=w_ld_chr, no_check_alleles=True
        )
        # Male_mysumstats.estimate_h2_by_ldsc(
        #     ref_ld_chr=ref_ld_chr, w_ld_chr=w_ld_chr, no_check_alleles=True
        # )
        ...
        h2_df = pd.concat(
            [
                # Male_mysumstats.ldsc_h2.assign(Trait=trait, Group="Male").drop(
                    # columns=["Catagories"]
                # ),
                mysumstats.ldsc_h2.assign(Trait=trait, Group="All").drop(
                    columns=["Catagories"]
                )
            ]
        )
        ...
        basic_cols = [
            "h2_obs_All", "h2_se_All",
            # "h2_obs_Female", "h2_se_Female", "h2_obs_Male", "h2_se_Male"
            ]
    ...
    # )
    # ldsc_rg_df = mysumstats.ldsc_rg.copy()
    # ldsc_rg_df.insert(0, "Trait", trait)

    # rg = mysumstats.ldsc_rg["rg"][0]
    # se_rg = mysumstats.ldsc_rg["se"][0]
