...
def process_trait(trait_params):
    ...
    chr_list_df = trait_list_df

    trait_save_folder = saveDir / trait
    trait_save_folder.mkdir(parents=True, exist_ok=True)

    sex_trait_results_saveDir = trait_save_folder / f"{trait}.feather"
    ...
    for regenie_result_file in tqdm(
        chr_list_df["file_dir"], desc=f"Loading {trait}"
    ):
        #
        if not is_burden:
            ...
            trait_chr_res_df = pd.read_csv(
                regenie_result_file, sep=r"\s+", comment="#"
            )

        sex_res_list.append(trait_chr_res_df)

        trait_res_df = pd.concat(sex_res_list).reset_index(drop=True)

        if not is_burden:
            trait_res_df = trait_res_df.drop(columns=["EXTRA"])
        ...
        print(
            f"Finished {trait} and saved to {sex_trait_results_saveDir} with time {duration:.2f} s"
        )
...
if len(chains) == 2:
    ...
    versionConvert = False
    from_version, to_version = None, None
    pass
...
regenie_results_files = [f for f in tgt_dir.glob("*.regenie") if f.is_file()]
